...
function get_id(){
    $strSQL = "SELECT id,name FROM genders WHERE name = '".$this->name."'";
    ...
    if ( mysql_num_rows($rsRES) > 0 ){
        ...
        $this->error_number = 1;
        $this->error_description="This Gender doesn't exist";
        return false;
    }
}
...
function get_detail(){
    $strSQL = "SELECT id,name,description FROM genders WHERE id = '".$this->id."'";
    $rsRES = mysql_query($strSQL,$this->connection) or die ( mysql_error() . $strSQL );
    if ( mysql_num_rows($rsRES) > 0 ){
        $this->id = mysql_result($rsRES,0,'id');
        $this->name = mysql_result($rsRES,0,'name');
...
function update(){
    ...
    $strSQL = "INSERT INTO genders (name, description) VALUES ('";
    ...
    elseif($this->id > 0 ) {
    $strSQL = "UPDATE genders SET name = '".addslashes(trim($this->name))."',";
    ...
    $rsRES = mysql_query($strSQL,$this->connection) or die(mysql_error(). $strSQL );
            if ( mysql_affected_rows($this->connection) >= 0 ) {
                ...
                $this->error_number = 3;
                $this->error_description="Can't update this Gender";
                return false;
            }
    }
}
...
function get_list_array(){
        $genders = array();$i=0;
        $strSQL = "SELECT id,name,description FROM genders ORDER BY id";
        $rsRES = mysql_query($strSQL,$this->connection) or die(mysql_error(). $strSQL );
        if ( mysql_num_rows($rsRES) > 0 ){
            while ( list ($id,$name,$description) = mysql_fetch_row($rsRES) ){
                $genders[$i]["id"] =  $id;
                $genders[$i]["name"] = $name;
                $genders[$i]["description"] = $description;
                $i++;
            }
            return $genders;
        }else{
        $this->error_number = 4;
        $this->error_description="Can't list Gender";
        return false;
        }
}
...
function get_list_array_bylimit($id=-1,$name="", $description="", $start_record = 0,$max_records = 25){
        ...
        $strSQL = "SELECT id,name,description FROM genders WHERE 1 ";
        if ( $id != "" && $id != -1 ) {
...
function delete(){
    if($this->id > 0 ) {
        $strSQL = " DELETE FROM genders WHERE id = '".$this->id."'";
        $rsRES = mysql_query($strSQL,$this->connection) or die(mysql_error(). $strSQL );
        if ( mysql_affected_rows($this->connection) > 0 ) {
            return true;
        }
            ...
            $this->error_number = 6;
            $this->error_description="Can't deltete this Gender";
            return  false;
        }
    }
}
